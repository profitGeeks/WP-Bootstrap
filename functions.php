...
function wp_bootstrap_custom_admin_footer() {
	echo '<span id="footer-thankyou">Developed by <a href="http://profitgeeks.org" target="_blank">Profitgeeks</a></span>';
}
...
if( !function_exists("wp_bootstrap_theme_styles") ) {  
    function wp_bootstrap_theme_styles() { 
        ...
        wp_register_style( 'wpbs', get_template_directory_uri() . '/library/css/wp.css', array(), '1.0', 'all' );
...
// add responsive to thumbs
add_filter('post_thumbnail_html','add_class_to_thumbnail');
function add_class_to_thumbnail($thumb) {
	if( true ) $thumb = str_replace('attachment-', 'img-responsive attachment-', $thumb);
	return $thumb;
}
